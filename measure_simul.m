...
disp(['r=', num2str(r), ' total spin:', num2str(Ns)])

end

function [tab, r, bond] = measure(x, y, tab, r, dims)
cir = dims(0);
wid = dims(1);
Ns = cir*wid*2;
assert(Ns*2 == size(tab, 2));

% Site index: x = 1~cir; y = 1~wid; ab = 0,1
pos = @(x, y, ab) wid.*(x-1) + y + ab.*dim;

% Generate bond
bond = randi([1, 3]); % 2:x, 1:z, 3:y
...
if phase == 1
    % Swap it with the new Pauli and modify the rest anti-commuting rows
    row_discard = tab(idx, :);
    tab(idx, :) = row;
    idx = idx + 1;
    while idx <= r
    ...
    tab_aug = [tab; row];
    tab_aug_galois = gf(tab_aug, 1, 2);
    if rank(tab_aug_galois) == r + 1
        tab = tab_aug;
        r = r + 1;
    end
end
