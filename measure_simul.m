function [] = measure_simul(varargin)
    ...
    for step = 1:T
        ...
        % Plotting
        if plotting
            plot(step, Ns - r, '*', 'Color', 'b');
            pause(0.01);
        end
    end
    
    disp(['r=', num2str(r), ' total spin:', num2str(Ns - 2*strcmp(boundary, 'open'))])
...
function [tab, r_stab_size, bond] = measure(x, y, tab, r_stab_size, pars)
    ...
    if phase == 1
        ...
        tab(r_stab_size+1, :) = row;
        tab_galois = gf(tab(1: r_stab_size+1, :), 1, 2);
        if rank(tab_galois) == r_stab_size + 1
            r_stab_size = r_stab_size + 1;
        end
    end
end
